...
しかし、ScreenC には半年以上変更が施されておらず、手元で再現できなかったため、難航することが予想されました。

### リリース内容の確認
このクラッシュは直近のバージョンから爆発的に発生するようになっていたことから、そのリリースに原因があると仮設を立て、調査を開始します。  
すると、ScreenA にログ収集処理の追加を行っていたことが分かりました。しかし、ScreenC でクラッシュを発生させるような依存は無く、実装された処理も問題があるようには見えませんでした。

### 原因箇所の特定
ScreenA が最もクラッシュ箇所に近いですが、因果関係がはっきりしない以上、他の変更箇所が影響している可能性を捨てることができませんでした。そこで、ScreenA に施した処理を全て元に戻して段階リリースを行いました。  
すると、該当のクラッシュは1件も発生しなくなり、ScreenA に施した処理が原因であることが確定しました。まだ具体的なクラッシュ原因は分かっていませんが「ログ収集処理が原因」というところまで分かったことにより、修正対応の優先度を決めて計画的に進めることが可能となりました。

### 根本的解決
調査の結果から、ScreenA と ScreenC は別物であるが、何かしら影響し合っているのだと仮定して調査することで多くの仮説を立てることができました。  
結論としては、ScreenA で行うログ収集処理の実行タイミングが適切ではなく、ScreenC でモーダルを閉じる、つまりモーダル上の画面全てのインスタンスを破棄することに失敗してクラッシュすることがあるというものでした。

今回は、結果的には実装にミスがあり、それに気づくことができなかったというものでした。しかし、適切な順番で原因である可能性の幅を小さくしていくことで、確実に答えへと近づくことができるのです。

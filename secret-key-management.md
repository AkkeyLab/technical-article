...
上記のコードは、yshrsmzさんが提供する `BuildKonfig` （*1）を導入するための設定です。KMM プロジェクトの `build.gradle.kts` ファイルにこのコードを追加することで、 `BuildKonfig` を利用できるようになります。
...
次に、ローカル環境で環境変数を管理する方法について説明します。もっとも簡単な方法は、上記のように `.zshrc` など、お使いのシェルの RC ファイルに直接記述する方法です。ただし、この方法ではローカル環境内でどこからでも簡単にアクセスできてしまうため、セキュリティ上の懸念があるかもしれません。その場合、 `direnv` というツールを使用することをおすすめします。このツールを使うと、ディレクトリごとに異なる環境変数を設定できるようになります。

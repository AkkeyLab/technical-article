...
シークレットキーをローカルマシンの Keychain Access アプリに保存するためには、GUI または `security` コマンドを使用します。チーム開発を行う環境では、1password などのパスワード管理ソフトウェアにシークレットキーと `security` コマンドをメモしておくと便利です。また、 `security` コマンドの詳細は、`man security` コマンドを使用することで調べることができます。  
ただし、CI（継続的インテグレーション）環境上ではこのコマンドを実行しないでください。ちなみに、Xcode Cloud では `security` コマンドの使用が禁止されています。
...
まず、Keychain Access に保存されたシークレットキーを取得する方法を確認しましょう。実は非常にシンプルで、保存時と同じ `security` コマンドを使用して取得できます。上記のスクリプトは、 `security` コマンドを使用してシークレットキーを取得するコマンドの一例です。各オプションに指定する値は適宜実際の値に置き換えてください。  
...
上記のスクリプトでは、 `-c` オプションの後に続く1つ目の文字列で編集内容を指定します。この場合、 `Set:` を使って値の書き込みを指定し、編集対象のキーを `LSEnvironment:NATIONAL_TAX_AGENCY_API_KEY` と指定しています。 `${NTA_API_KEY}` はシェル変数であり、「Xcode Cloud 対応」で詳しく解説します。  
...
## KMM 対応
KMM（Kotlin Multiplatform Mobile）は、ビジネスロジックを Kotlin で記述し、iOS と Android の両方で使用できる仕組みです。KMM は、UI 設計が共通化されていないという点で他のマルチプラットフォームと異なっています。  
シークレットキーを利用する処理は、通常ビジネスロジックに含まれることが多いため、これまでに紹介した方法を KMM にそのまま適用するのは難しいかもしれません。そのため、KMM では以下の2つのステップでシークレットキーの管理環境を整えることにします。
...
もし `BuildConfig` クラスでシークレットキーの取得と定義ができれば、アプリのコードから直接シークレットキーを参照せずに済むため、安全性を向上させることができるでしょう。しかし、本稿執筆時点で KMM には `BuildConfig` のような機能が実装されていません。  
...
最も重要な部分は、 `buildConfigField` 関数です。第一引数に変数の型を指定し、第二引数に変数名を文字列で指定します。そして、第三引数には実際のシークレットキーを文字列で指定します。ただし、直接シークレットキーを記述すると、コミットにシークレットキーが含まれてしまうため、適切なセキュリティ対策が必要です。
...
ローカルの環境変数を取得する処理を Kotlin で実装すると上記のようになります。この方法を使うと、特に追加の対応を必要とせずに、ローカル環境と CI 環境の両方で動作が可能となります。

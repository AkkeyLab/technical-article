...
ここからは、あなたがエンジニアとしてAppleに入社したと仮定してデリゲーションパターンの実現についてご紹介します。そんなあなたに「`IndexPath(item: 0, section: 0)`のセルだけを強調表示する機能をUICollectionViewに実装してほしい」というお仕事が舞い込んできたらどのように実装しますか？もちろん、それだけだと仕様として情報不足すぎですし、そもそもUIKit内部のコードは公開されていないのでイメージだけで問題ありません。まず、全体のイメージとしては、以下のようになるでしょう。
